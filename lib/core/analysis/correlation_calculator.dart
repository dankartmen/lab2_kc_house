...
class CorrelationCalculator {
  ...
  static Map<String, Map<String, double>> calculateMatrix<T extends DataModel>(
    ...
    if (data.isEmpty || fields.isEmpty) return {};
    ...
    for (final field1 in fields) {
      for (final field2 in fields) {
        matrix[field1]![field2] = _calculateFieldCorrelation(data, field1, field2);
      }
    }
  ...
  /// 
  /// Возвращает:
  /// - [double] — коэффициент корреляции Пирсона в диапазоне [-1, 1].
  /// 
  /// Возвращает 0.0 если недостаточно данных или произошла ошибка.
  static double _calculateFieldCorrelation<T extends DataModel>(
    ...
    if (values1.length < 2) return 0.0; // Недостаточно данных для корреляции
  ...
  static double _calculatePearsonCorrelation(List<double> x, List<double> y) {
    if (x.length != y.length) throw ArgumentError('Arrays must have the same length');
